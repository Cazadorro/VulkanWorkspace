    ...
    public:
        ...
        CodeGenerated visit(const Expression &expr) {
            ...
            for (const auto &rule: expr.exprs) {
                ...
                generated_code +=
                        fmt::format("    template<>\n"
                                    "    tl::expected<{0}*, ParseError<{1}> >\n"
                                    "    consume(ParseState<{1}> &parse_state, MemoizationState &memoization_state);\n", rule->non_terminal.lexeme(), "name");
            }
            // template<typename T>
        ...
        CodeGenerated visit(const RuleDefinitionExpr &expr) {
            ...
            std::string top_code =
                            fmt::format("{0}template<>\n"
                                        "{0}tl::expected<{1}*, ParseError<{2}> >\n"
                                        "{0}consume(ParseState<{2}> &parse_state, MemoizationState &memoization_state){{\n", indent, expr.non_terminal.lexeme(), "name");
