...
gul::bitmask::bitmask(std::size_t size_bits) : m_data(size_bits % m_bit_count == 0 ? size_bits/m_bit_count : (size_bits/m_bit_count) + 1, 0){
...
const std::uint32_t *gul::bitmask::data() const {
    return m_data.data();
}

std::size_t gul::bitmask::size() const {
    return m_data.size();
}
