...
def main():
    ...
    vertex_x = np.array([int(Nx * 2 / 4), int(Nx * 3 / 4), int(Nx * 3 / 4), int(Nx * 2 / 4)])
    vertex_y = np.array([int(Ny * 2 / 4), int(Ny * 2 / 4), int(Ny * 3 / 4), int(Ny * 3 / 4)])

    # vertices = np.hstack(vertex_x,vertex_y)
    vertices = np.concatenate((vertex_x[:, np.newaxis], vertex_y[:, np.newaxis]), axis=1)
    average_node_distance = 0
    for i in range(len(vertices)):
        a = vertices[i]
        b = vertices[(i + 1) % len(vertices)]
        distance = np.linalg.norm(a-b)
        average_node_distance += distance
    average_node_distance /= len(vertices)
    vertices_origional = vertices
    ...
    for it in range(Nt):
        ...
        node_forces = calc_force_at_nodes(vertices, vertices_origional, transform, average_node_distance, dx, spring_strength)
        ...
        for node_force, vertex in zip(node_forces, vertices):
            x = int(vertex[0])
            y = int(vertex[1])
            for x >= 0 and x < map_shape[1] and y >= 0 and y < map_shape[1]
        for i, cx, cy, w in zip(idxs, cxs, cys, weights):
            forces[i, :, :] += calc_force_i_2(w,cx,cy,np.array([force_x,force_y]))
